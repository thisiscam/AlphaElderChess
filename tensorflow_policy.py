...
class PolicyValueNet():
    def __init__(self, model_file=None, server=None):
        ...
        with self.graph.as_default():
            self.hidden_pieces_state = tf.placeholder(
                    tf.float32, shape=[None, 2, 4],
                    name="hidden_pieces_state"
                )
            ...
            self.action_conv_flat = tf.reshape(
                    self.action_conv, [-1, 6 * self.board_height * self.board_width])
            ...
            self.evaluation_fc1 = tf.layers.dense(inputs=self.evaluation_conv_flat_aug,
    ...
    def policy_value(self, state_batch):
        ...
        log_act_probs, value = self.session.run(
                ...
                feed_dict={
                            self.input_states: state_batch[0],
                            self.hidden_pieces_state: state_batch[1],
                            self.remaining_steps_state: state_batch[2]
                           }
                )
    ...
    def train_step(self, state_batch, mcts_probs, winner_batch, lr):
        ...
        loss, entropy, _ = self.session.run(
                ...
                feed_dict={self.input_states: state_batch[0],
                           self.hidden_pieces_state: state_batch[1],
                           self.remaining_steps_state: state_batch[2],
