...
def mcts_player_at_branch(git_tag, n_playout, name='', tmp_repo_path='./tmp/AEC'):
    build_dir = "build_" + git_tag
    subprocess.check_call(['git', 'fetch', '--all', '--tags', '--prune'], cwd=tmp_repo_path)
    subprocess.check_call(['git','checkout', 'tags/'+git_tag], cwd=tmp_repo_path)
    ...
    spec1 = importlib.util.spec_from_file_location(git_tag + "_module", os.path.join(tmp_repo_path, 'mcts_player.py'))
    player_module = importlib.util.module_from_spec(spec1)
    spec1.loader.exec_module(player_module)

    spec2 = importlib.util.spec_from_file_location(git_tag + "_module", os.path.join(tmp_repo_path, 'tensorflow_policy.py'))
    network_module = importlib.util.module_from_spec(spec2)
    spec2.loader.exec_module(network_module)

    value_net = network_module.PolicyValueNet(model_file=os.path.join("models_"+git_tag, 'best_policy.model'))
